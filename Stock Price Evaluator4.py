...
Upload an Excel file with stock tickers to analyze valuation based on inflation-adjusted cash flow fundamentals.
""")
...
def get_inflation_rate():
    """Fetch current inflation rate from FRED (CPI)"""
    try:
        cpi = fred.get_series('CPIAUCSL')
        latest_cpi = cpi[-1]
        year_ago_cpi = cpi[-13]  # 12 months ago
        inflation_rate = (latest_cpi - year_ago_cpi) / year_ago_cpi
        return inflation_rate
    except:
        return 0.025  # Default to 2.5% if API fails
...
with st.sidebar:
    ...
    adjust_for_inflation = st.checkbox(
        "Adjust for inflation (real cash flows)",
        value=True,
        help="Use inflation-adjusted cash flows and real discount rates"
    )
...
def calculate_dcf(fcf, growth_rate, discount_rate, growth_period, terminal_growth, inflation_rate=0.0):
    ...
    real_discount_rate = discount_rate - inflation_rate
    real_growth_rate = growth_rate - inflation_rate
    real_terminal_growth = terminal_growth - inflation_rate
    
    # Projected cash flows during growth period
    for year in range(1, growth_period + 1):
        future_fcf = fcf * (1 + real_growth_rate) ** year
        present_value += future_fcf / ((1 + real_discount_rate) ** year)
    
    # Terminal value
    terminal_fcf = fcf * (1 + real_growth_rate) ** growth_period
    terminal_value = (terminal_fcf * (1 + real_terminal_growth)) / (real_discount_rate - real_terminal_growth)
    present_terminal_value = terminal_value / ((1 + real_discount_rate) ** growth_period)
...
def get_stock_data(ticker, period, discount_method, manual_discount_rate, growth_period, terminal_growth, adjust_for_inflation, inflation_rate):
    try:
        ...
        if discount_method == "Use industry default":
            ...
            nominal_rate = manual_discount_rate / 100
        ...
        data = {
            ...
            "Inflation Adjusted": "Yes" if adjust_for_inflation else "No"
        }
        ...
        if not np.isnan(free_cash_flow) and free_cash_flow > 0:
            ...
            intrinsic_value = calculate_dcf(
                ...
                growth_period, 
                terminal_growth/100,
                inflation_rate if adjust_for_inflation else 0.0
            )
...
def scan_tickers(tickers, period, discount_method, manual_discount_rate, growth_period, terminal_growth, adjust_for_inflation, inflation_rate):
    results = []
    failed_tickers = []
    ...
    try:
        ...
        if not tickers:
            ...
            if results:
                ...
                st.markdown(f"**Inflation adjustment:** {'Enabled' if adjust_for_inflation else 'Disabled'} (Current inflation: {current_inflation:.1%})")
                ...
                with st.expander("DCF Assumptions"):
                    ...
                    st.write(f"**Discount Rate:** {selected_data['Suggested Discount Rate']:.1f}%")
                    if adjust_for_inflation:
                        st.write(f"**Real Discount Rate:** {float(selected_data['Suggested Discount Rate']) - current_inflation*100:.1f}% (adjusted for inflation)")
                    st.write(f"**Terminal Growth Rate:** {terminal_growth:.1f}%")
                    if adjust_for_inflation:
                        st.write(f"**Real Terminal Growth:** {terminal_growth - current_inflation*100:.1f}%")
