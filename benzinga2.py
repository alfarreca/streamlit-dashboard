...
# New XLSX uploader
uploaded_file = st.sidebar.file_uploader("Upload XLSX file (optional)", type=["xlsx"])
uploaded_df = None
...
# Pick which DataFrame to use (uploaded or from Yahoo)
if uploaded_df is not None:
    df = uploaded_df.copy()
    st.subheader("📂 Uploaded XLSX Data")
    st.dataframe(df)
else:
...
# --- Download XLSX button (for either dataframe) ---
def to_excel(df):
    output = BytesIO()
...
if st.sidebar.button("Download XLSX file"):
    if uploaded_df is not None:
        xlsx_data = to_excel(uploaded_df)
        st.download_button(
            label="Download Uploaded Data as XLSX",
            data=xlsx_data,
            file_name="uploaded_data.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="download_uploaded_xlsx"
        )
    else:
        xlsx_data = to_excel(df)
        st.download_button(
            label="Download Yahoo Data as XLSX",
            data=xlsx_data,
...
if uploaded_df is None and st.sidebar.button("Export as CSV"):
    csv = df.to_csv(index=True).encode('utf-8')
    st.download_button("Download Price Data CSV", csv, f"{ticker}_history.csv", "text/csv", key='csv_dl')

# --- If uploaded, skip metrics and chart. If Yahoo, proceed as before ---
if uploaded_df is not None:
    st.info("App metrics and charting are based on Yahoo data. Re-upload with correct columns if you want analysis on uploaded data.")
    st.stop()
...
normalized_vol = avg_volume / 1e6  # Millions
volume_pct = avg_volume / max(df['Volume'].max(), 1)  # as a percent of max vol in lookback
...
def get_options_flow(ticker):
    try:
        ...
        if not expirations:
            return 1.0, None, None, 0, 0  # Neutral + 0 activity
        # Use nearest expiry options chain
...
def calculate_levels(price, volume, options_ratio, atr, vol_scale, atr_scale, bullish=True):
    # Use normalized (dimensionless) factors
    if bullish:
...
if calls is not None and puts is not None:
    ...
    if total_opt_activity > avg_volume:  # crude signal: options vol > stock vol
