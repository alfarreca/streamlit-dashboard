...
def display_results(filtered_df):
    ...
    # **Add "Company Name" column to table view**
    st.dataframe(
        filtered_df[[
            "Symbol", "Exchange", "Company Name", "Price", "5D_Change", "20D_Change",
...
def main():
    ...
    company_file = st.file_uploader("Upload Excel file with company names (e.g. Hang_Seng.xlsx)", type="xlsx")
    ...
    # --- Load Company Names (Hang Seng file) ---
    if company_file is not None:
        df_companies = pd.read_excel(company_file)
        # Try to be robust: look for "Company Name" or similar
        name_col = None
