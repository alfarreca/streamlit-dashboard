...
def get_news_yfinance(ticker):
    try:
        ...
        return [{
            ...
            'date': datetime.fromtimestamp(item.get('providerPublishTime', time.time())).strftime('%b %d, %Y')
        } for item in news[:3]]
    except Exception:
        return []
...
def render_single_company(tickers, selected_miners, gold_price, discount_rate):
    ...
    with col1:
        ...
        if gold_price is not None:
            ...
            cols[0].metric(
                f"NAV ({int(discount_rate*100)}% discount)",
                format_metric(nav, 'currency'),
                help=f"Net Asset Value: Reserves discounted by {int(discount_rate*100)}%"
            )
            cols[1].metric(
                "EV/Production",
            ...
            if nav and fundamentals.get('Market Cap'):
                ...
                <div class="metric-card {'mining-metric' if premium_discount < 0 else ''}">
                    ...
                    <p style='font-size:smaller;'>NAV Discount Rate: {discount_rate*100:.1f}%</p>
                </div>
                """, unsafe_allow_html=True)
        else:
    ...
    with col2:
        st.subheader("Recent News (Google News + Yahoo Finance fallback)")
        company_display = selected_company.split(" (")[0]
        ticker = tickers[selected_company]
        with st.spinner("Fetching news..."):
            news = get_news_google(company_display)
            if not news:
        ...
        if not news:
            ...
            for item in news:
                ...
                <div class="metric-card">
                    ...
                    <small>{item.get('publisher',news_source)} • {item.get('date','')}</small>
                </div>
                """, unsafe_allow_html=True)
...
def render_multi_company(tickers, selected_miners, discount_rate):
    ...
    for i, company in enumerate(selected_miners):
        ...
        with st.spinner(f"Loading {company} data..."):
            try:
                ...
                mining_metrics = get_mining_metrics(ticker)
                if fundamentals:
                    nav = calculate_nav(ticker, gold_price, discount_rate) if gold_price else None
                    ...
                    combined_data = {
                        **fundamentals,
                        **mining_metrics,
                        f'NAV ({int(discount_rate*100)}%) (B)': nav / 1e9 if nav else None,
    ...
    valuation_metrics = [f'NAV ({int(discount_rate*100)}%) (B)', 'EV/Production', 'Value/Reserve oz', 'CF Multiple']
    ...
    if selected_valuation_metrics:
        ...
        val_format_dict = {
            f'NAV ({int(discount_rate*100)}%) (B)': '${:,.2f}',
            'EV/Production': '{:.1f}x',
            'Value/Reserve oz': '${:,.0f}',
            'CF Multiple': '{:.1f}x'
        }
    ...
    # Recent News for All Companies
    st.subheader("Recent News (All Companies)")
    with st.expander("Show recent headlines for all selected companies"):
        for company in selected_miners:
            company_display = company.split(" (")[0]
            ticker = tickers[company]
            news = get_news_google(company_display)
            if not news:
                news = get_news_yfinance(ticker)
                ...
                news_source = "Google News"
            st.markdown(f"**{company_display}** — <span style='font-size:small;color:gray;'>{news_source}</span>", unsafe_allow_html=True)
            if not news:
...
def main():
    ...
    with st.sidebar:
        ...
        discount_rate = st.slider(
            ...
            help="Annual rate used to discount future mining cash flows. Higher = more risk assumed. Default: 5%"
        ) / 100.0
    ...
    if analysis_type == "Single Company":
        render_single_company(tickers, selected_miners, gold_price, discount_rate)
    else:
        render_multi_company(tickers, selected_miners, discount_rate)
