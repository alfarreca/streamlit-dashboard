...
def find_first_trigger_date(hist, threshold):
    for i in range(50, len(hist)):
        window = hist.iloc[:i+1]
        score = calculate_momentum(window)["Momentum_Score"]
        if score >= threshold:
            return hist.index[i].strftime("%Y-%m-%d")
    return None
...
def main():
    ...
    # --------- MOVED ALERTS LOGIC INSIDE main() ---------
    ...
    if not alerts.empty:
        ...
        for date, symbol, score in sorted(alerts_with_dates, reverse=True):
            st.success(f"{symbol} triggered on {date} with Momentum Score = {score}")
            # Optional: Visualize chart with marker on trigger date
            try:
